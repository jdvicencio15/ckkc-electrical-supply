...
function Purchases() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      console.error(`Failed to ${type} purchase:`, error);

      setToast({
        type: "error",
        message: error.response?.data?.message || `Failed to ${type} purchase.`,
      });
    } finally {
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        confirmText={
          confirmAction?.type === "delete" ? "Delete" : "Receive Purchase"
        }
        cancelText="Cancel"
        loading={confirmLoading}
        loadingText={
          confirmAction?.type === "delete" ? "Deleting..." : "Receiving..."
        }
      />
                      ...
                      {[...filteredPurchases]
                        .sort((a, b) => {
                          const sequenceA = Number(
                            a.purchaseNumber?.split("-").pop(),
                          );
                          const sequenceB = Number(
                            b.purchaseNumber?.split("-").pop(),
                          );
                        ...
                        .map((purchase) => (
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              <span
                                className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${purchase.status === "received"
