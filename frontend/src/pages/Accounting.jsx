...
function Accounting() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [accountsResponse, journalEntriesResponse, trialBalanceResponse] =
        await Promise.all([
      ...
      const journalData =
        journalEntriesResponse?.data || journalEntriesResponse || [];
      ...
      setTrialBalance({
        totalDebit: Number(trialBalanceData.totalDebit || 0),
        totalCredit: Number(trialBalanceData.totalCredit || 0),
        isBalanced: Boolean(trialBalanceData.isBalanced),
      });
    } catch (err) {
      console.error(err);

      setError(
        err.response?.data?.message || "Failed to load accounting dashboard.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const financialSummary = useMemo(() => {
    ...
    return {
      ...
      netIncome: Number((totals.revenue - totals.expenses).toFixed(2)),
    };
  }, [journalEntries]);
    ...
    for (const journalEntry of journalEntries) {
      for (const entry of journalEntry.entries || []) {
        ...
        if (
          ...
          balances[accountId].balance += credit - debit;
        }
      }
    }
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">
            {loading
              ? "..."
              : formatCurrency(financialSummary.assets, settings?.currency)}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">
            {loading
              ? "..."
              : formatCurrency(financialSummary.revenue, settings?.currency)}
          </p>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">
            {loading
              ? "..."
              : formatCurrency(financialSummary.expenses, settings?.currency)}
          </p>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="divide-y divide-slate-200 dark:divide-slate-800">
              {journalEntries.slice(0, 5).map((entry) => {
                const amount = (entry.entries || []).reduce(
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-semibold ${
                trialBalance.isBalanced ? "text-emerald-600" : "text-red-500"
              }`}
            >
              {trialBalance.isBalanced
