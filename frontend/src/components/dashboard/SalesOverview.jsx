...
function SalesOverview({ sales = [] }) {
  ...
  const currentSales = getSalesTotal(currentStartDate, currentEndDate);

  const previousSales = getSalesTotal(previousStartDate, previousEndDate);
  ...
  if (percentageChange > 0) {
    changeLabel =
      previousSales === 0 ? "↑ New sales" : `↑ ${roundedPercentage}%`;
  ...
  if (percentageChange < 0) {
    ...
    changeClass = "bg-red-50 text-red-600 dark:bg-red-950/40 dark:text-red-400";
  }
  ...
  if (selectedPeriod === "last-3-months") {
    for (let i = 2; i >= 0; i--) {
      ...
      const monthSales = getSalesTotal(monthStart, monthEnd);
  ...
  const maxSales = Math.max(...salesData.map((data) => data.sales), 1);
      ...
      <div className="mb-5">
        ...
        <div className="mt-1 flex items-center gap-3">
          <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100">
            {formatCurrency(currentSales, settings?.currency)}
          </h3>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    height: `${Math.max(height, item.sales > 0 ? 5 : 0)}%`,
                  }}
                  title={formatCurrency(item.sales, settings?.currency)}
                />
              </div>
            );
          })}
